...
export class AuthenticationService {
  ...
  constructor(private router: Router, private http: HttpClient) {
    ...
    this.currentUserSubject = new BehaviorSubject<User>((user ? JSON.parse(user) : {Id: 0, FirstName: '', LastName: '', Email: ''}))
  ...
  login(user: Partial<User>): void {
    ...
    this.http.post<LoggedInResponse>(environment.api + '/user/authenticate', {Username: user.Email, Password: user.Password}).pipe(catchError((error: HttpErrorResponse) => this.handleError(error))).subscribe((data: LoggedInResponse) => {
      localStorage.setItem('authorization', 'Bearer ' + data.AccessToken)
      this.http.get<User>(environment.api + '/user/' + data.Id,  {headers: {authorization: localStorage.getItem('authorization')!}}).subscribe(data => {
  ...
  logout():void {
    ...
    this.updateUser({Id: 0, FirstName: '', LastName: '', Email: ''})
